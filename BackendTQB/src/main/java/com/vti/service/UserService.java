...
public class UserService implements IUserService {
    ...
    @Override
    public List<UserDTOv2> findAllUser() {
        List<User> users = userRepository.findAll();
        List<UserDTOv2> dtos = new ArrayList<>();
        for (User user : users) {
            UserDTOv2 dto = UserDTOv2
                    .buildUser(user.getId(),
                            user.getUserName(),
                            user.getEmail(),
                            user.getFirstName(),
                            user.getLastName()
...
//    public List<UserDTOv2> findAllUser() {
//        List<User> users = userRepository.findAll();
//        List<UserDTOv2> dtos = mapper.map(users, new TypeToken<List<UserDTOv2>>() {
//        }.getType());
//        return dtos;
//    }
